...
| `collab` | 콜라보 작품명 (예: "던전 밥"). 연표에 배지로 표시되고, 연도가 없으면 "콜라보" 구역에 모인다 |
...
## regions.json — 이벤트의 지역·세력

스토리 목록의 "팩션별 보기" 에 쓰는 분류다. 게임 데이터에는 이벤트가 어느 지역 이야기인지 알려 주는 항목이 없어서,
`scripts/process-data.mjs` 가 본문에 지역 이름(한국어·중국어 표기, 주요 도시 이름 포함)이 몇 번 나오는지 세어 추정한다.
가장 많이 나온 지역이 대표가 되고, 그 40% 이상 나온 지역도 함께 표시한다(최대 3개).

추정이 어색한 이벤트는 여기에 직접 적는다. 적으면 추정값을 무시하고 이 값만 쓴다.
...
첫 번째가 대표 지역이다. 지역 이름은 `scripts/process-data.mjs` 의 `REGION_WORDS` 에 있는 것을 쓴다.
